...
Limites desta versão: um operador por vez na mesma máquina; imagens PNG, JPEG ou WebP de até 10 MB e 40 megapixels, até 12 por seção de evidências; arquivos anexados aos passos de até 20 MB cada, 20 por passo. Imagens embutidas na planilha não são importadas.

## Gerar o executável

O `.exe` não faz parte do repositório. Para gerá-lo no Windows (Python 3.12):

```bat
pip install -r requirements.txt pyinstaller
pyinstaller --noconfirm --clean --windowed --name ExecutorLocal --add-data "testplayer/templates;testplayer/templates" --add-data "testplayer/static;testplayer/static" --distpath aplicativo launcher.py
```

O resultado fica em `aplicativo\ExecutorLocal\ExecutorLocal.exe` (pasta ignorada pelo Git).
