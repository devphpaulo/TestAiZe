...
Aplicativo local para Windows de execução de testes manuais: importe planilhas de casos (`.xlsx` / CSV), execute passo a passo com status, anotações e evidências e exporte relatórios em PDF ou HTML. Roda 100% offline em `127.0.0.1`, com ícone na bandeja do sistema — usando o `.exe`, não é preciso instalar Python.

## Funcionalidades

- Importação de `.xlsx` / CSV UTF-8 com reconhecimento automático de cabeçalhos
- Test player com status por passo, data/hora, anotações e evidências
- Imagens coladas com `Ctrl+V` e anexos de arquivos nos passos
- Reteste na mesma sessão, preservando execuções anteriores
- Filtro por pastas, barras de distribuição e modo Kanban
- Relatórios PDF e HTML (sessão completa, casos selecionados ou só a falha)
- Prompt de bug copiável para casos reprovados
- Tema claro/escuro e cor de destaque, lembrados pelo navegador
- Backup automático e dados persistidos fora da pasta do aplicativo

## Requisitos

- **Usando o `.exe` (recomendado):** Windows 10/11, nada mais.
- **Rodando do código-fonte:** Python 3.12+ e as dependências de `requirements.txt`.

## Instalação

### Executável via GitHub Actions

O `.exe` não faz parte do repositório; ele é gerado pelo workflow **Build exe**:

1. Abra a aba [Actions](https://github.com/devphpaulo/TestAiZe/actions) e execute o workflow **Build exe**;
2. Ao final da execução, baixe o artefato `TestAiZe-windows`;
3. Descompacte e dê dois cliques em `TestAiZe.exe`.
...
```
launcher.py                     Entrada: servidor local (waitress), bandeja, navegador
testplayer/
  web.py                        Rotas Flask (somente 127.0.0.1)
  storage.py                    Persistência SQLite, migrações e backups
  importer.py                   Importação .xlsx / CSV
  attachments.py                Anexos dos passos (nome, tipo, integridade)
  pdf_export.py                 Relatórios PDF
  html_export.py                Relatórios HTML autossuficientes
  bug_prompt.py                 Prompt de bug para casos reprovados
  backup.py                     Backup dos dados
  static/, templates/           Frontend
.github/workflows/build.yml     Build do executável (PyInstaller, GitHub Actions)
```
